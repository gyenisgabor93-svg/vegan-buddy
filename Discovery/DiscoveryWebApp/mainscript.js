...
subscriptionsBtn: "Memberships",
    ...
    //Aboutus
aboutGreeting: "Welcome 👋",
aboutText:
   "This is a small, values-driven project focused on education, awareness, and connection around veganism and conscious living.\n\nThe organization behind it is building educational tools, including this app and a connection platform for vegans, with the aim of helping people learn, reflect, and connect through shared values.\n\nAlongside the digital work, there is also street activism and awareness activities that explore how our treatment of animals relates to broader themes of ethics, history, and consciousness.",
aboutFollowText: "Follow our journey on ",
aboutInstagram: "@org.elu",
aboutEnding: "",
...
weeklyVote: "Weekly Vote Reminder",
...
deleteProfileBtn: "🗑️ Delete profile",
  ...
  startCheck: "Comenzar registro diario",
  skipCheck: "Omitir registro diario",
...
subscriptionsBtn: "Membresías",
    ...
    //Aboutus
aboutGreeting: "Bienvenido 👋",
aboutText:
  "Este es un pequeño proyecto basado en valores, centrado en la educación, la conciencia y la conexión en torno al veganismo y la vida consciente.\n\nLa organización detrás de este proyecto está creando herramientas educativas, incluyendo esta aplicación y una plataforma de conexión para personas veganas, con el objetivo de ayudar a aprender, reflexionar y conectar a través de valores compartidos.\n\nAdemás del trabajo digital, también se realizan actividades de activismo en la calle y de concienciación que exploran cómo nuestra relación con los animales se conecta con temas más amplios de ética, historia y conciencia.",
...
communityEvent: "Eventos de la comunidad",
dailyCheckin: "Recordatorio diario",
weeklyVote: "Votación semanal",
encouragement: "Mensajes de motivación",
...
subscriptionsBtn: "Tagságok",
    ...
    //Aboutus
aboutGreeting: "Üdvözöllek 👋",
aboutText:
  "Ez egy kisméretű, értékalapú projekt, amely az oktatásra, a tudatosságra és a kapcsolódásra fókuszál a veganizmus és a tudatos életmód körül.\n\nA projekt mögött álló szervezet oktatási eszközöket fejleszt, beleértve ezt az alkalmazást és egy vegánok számára készült kapcsolódási platformot, azzal a céllal, hogy segítse az embereket a tanulásban, az önreflexióban és a közös értékeken alapuló kapcsolódásban.\n\nA digitális munka mellett utcai aktivizmus és figyelemfelhívó tevékenységek is zajlanak, amelyek azt vizsgálják, hogyan kapcsolódik az állatokkal való bánásmód az etika, a történelem és a tudatosság tágabb kérdéseihez.",
...
friendRequest: "Baráti kérés",
communityEvent: "Közösségi esemény",
dailyCheckin: "Napi bejelentkezés emlékeztető",
weeklyVote: "Heti szavazás",
encouragement: "Mensajes de motivación",
...
document.getElementById("notificationsTitle").innerText = t.notificationsTitle;
document.getElementById("labelNewMessage").innerText = t.newMessage;
document.getElementById("labelFriendRequest").innerText = t.friendRequest;
document.getElementById("labelCommunityEvent").innerText = t.communityEvent;
document.getElementById("labelDailyCheckin").innerText = t.dailyCheckin;
document.getElementById("labelWeeklyVote").innerText = t.weeklyVote;
document.getElementById("labelEncouragement").innerText = t.encouragement;
...
import { onboardingLessonPool } from './scriptpools.js';
import { onboardingSecondLessonPool } from './scriptpools.js';
import { onboardingFlow } from './scriptpools.js';
import { onboardingFlowSecond } from './scriptpools.js';
...
let currentConnectionRecommendationProbability = 0;
...
popularRecipeLoading: "🍲 Finding a trending recipe…",
popularRecipeLoadingInfo: "We are gathering the most loved recipes from the community.",
popularRecipeInfo: "One of the top-liked recipes in our community right now.",
popularRecipeGoTo: "👉 View recipes",

nextLessonTitleNewsBox: "📚 A new lesson is waiting for you: ",
goToLessons: "👉 Go to lessons",

connectionAppRecommendation: "Looking for vegan friends, a supportive community, or even love? Discover people who truly share your values with our connection app!"
...
popularRecipeLoading: "🍲 Buscando una receta popular…",
popularRecipeLoadingInfo: "Estamos recopilando las recetas más queridas por la comunidad.",
popularRecipeInfo: "Una de las recetas más populares en este momento.",
popularRecipeGoTo: "👉 Ver las recetas",

nextLessonTitleNewsBox: "📚 Te espera una nueva lección: ",
goToLessons: "👉 Ir a lecciones",

connectionAppRecommendation: "¿Buscas amigos veganos, una comunidad de apoyo o incluso una pareja? Encuentra personas que realmente comparten tus valores en nuestra app de conexión!"
},
  hu: {
...
popularRecipeLoading: "🍲 Egy népszerű recept keresése…",
popularRecipeLoadingInfo: "A közösség legkedveltebb receptjeit gyűjtjük össze.",
popularRecipeInfo: "A közösség egyik legnépszerűbb receptje.",
popularRecipeGoTo: "👉 Receptek megtekintése",

nextLessonTitleNewsBox: "📚 Egy új lecke vár rád: ",
goToLessons: "👉 Ugrás a leckékhez",

connectionAppRecommendation: "Vegán barátokat, támogató közösséget vagy akár párt keresel? Találd meg a társaidat a kapcsolatépítő appunkban!"
}
};
...
if (
  ...
  const { data: probData, error: probError } = await supabase
    ...
    .select("connection_app_recommendation_probability")
    .single();

  if (probError) {
    console.error("Error fetching connection probability:", probError);
  } else {
    currentConnectionRecommendationProbability =
      probData?.connection_app_recommendation_probability ?? 0;
  }
} else {
  currentConnectionRecommendationProbability = 0;
}
...
const next = getNextLessonFromPool(currentProfile);

const nextLesson = next.lesson;
const isOnboarding = next.isOnboarding;
...
if (xpToNextEl) {
  ...
  const petAvatarDCI = document.getElementById("petAvatarDCI");
...
function getNextLessonFromPool(profile) {
  ...
  const day = profile.day_counter || 0;

  // 🟢 Onboarding phase (first days)
  if (day === 0) return { lesson: onboardingStaticLessons.intro, isOnboarding: true };
  if (day === 1) return { lesson: getDietBasedOnboardingLesson(profile, 0).todayLesson, isOnboarding: true };
  if (day === 2) return { lesson: getDietBasedOnboardingLesson(profile, 1).todayLesson, isOnboarding: true };
  if (day === 3) return { lesson: onboardingStaticLessons.gameIntro, isOnboarding: true };
  if (day === 4) return { lesson: getDietBasedSecondOnboardingLesson(profile, 0).todayLesson, isOnboarding: true };
  if (day === 5) return { lesson: getDietBasedSecondOnboardingLesson(profile, 1).todayLesson, isOnboarding: true };

  // 🔵 Normal system after
  const next = basicDCI(profile);

  return {
    lesson: next.todayLesson,
    isOnboarding: false
  };
/*
  const defaultGoalOrder = [
...
async function recommendConnectionApp() {

  const probability = currentConnectionRecommendationProbability;
  ...
  // TEXT
  // ------------------------
  textEl.textContent = initT("connectionAppRecommendation");

  // ------------------------
  // REMOVE PET IMAGE USAGE
  // ------------------------
  if (photoEl) {
  ...
  // ------------------------
  // ICON UNDER TEXT (NEW ELEMENT)
  // ------------------------
  let appIconEl = document.getElementById("connectionAppIcon");

  if (!appIconEl) {
    appIconEl = document.createElement("img");
    appIconEl.id = "connectionAppIcon";

    textEl.insertAdjacentElement("afterend", appIconEl);
  }

  const appIconUrl =
    window.connectionAppIconUrl ??
    "https://pqrgvelzxmtdqrofxujx.supabase.co/storage/v1/object/public/system_photos/Connection%20Icon.jpeg";

  appIconEl.src = appIconUrl;
  ...
  appIconEl.onclick = () => {
    window.location.href =
      "https://vegan-buddy.vercel.app/connectionapp/index.html";    //REPLACE with play store link when available
  };
...

{
  ...
  value: 1,
  message: () => {
    const tips = characterTips.pet[currentLang] || characterTips.pet.en;
      ...
      (match, key) => {
        ...
        return `
          <div class="ingredient-inline">
            <strong>${item.name[currentLang] || item.name.en}</strong>
            <p>${item.description[currentLang] || item.description.en}</p>
            <small>${item.nutrition[currentLang] || item.nutrition.en}</small>
          </div>
        `;
      }
    );

    return randomTip; // ⚠️ returns STRING (like encouragement)
  }
},

{
  ...
  message: () => {
    ...
    const name = celeb.name[currentLang] || celeb.name.en;
    const title = celeb.title[currentLang] || celeb.title.en;
    const quote = celeb.quote[currentLang] || celeb.quote.en;

    const credibilityLabel =
      celeb.credibilityLabel?.[currentLang] ||
      celeb.credibilityLabel?.en ||
      "";
      ...
      <div class="celeb-inline">
        ...
        <div class="celeb-header">
          ...
          <div>
            <strong>${name}</strong>
            <p><em>${title}</em></p>
            <span style="color:${celeb.credibilityColor}">
              ${credibilityLabel}
            </span>
          </div>
        </div>
...

{
  ...
  message: () => {
    ...
    const learnMore = story.learnMore?.[currentLang] || story.learnMore?.en;

    return `
      <div class="success-story">
        <img src="${story.image}" class="success-image" />

        <h3>${story.title[currentLang] || story.title.en}</h3>

        <p>${story.story[currentLang] || story.story.en}</p>

        ${
          learnMore
            ? `<a href="${learnMore.url}" target="_blank">${learnMore.title}</a>`
            : ""
        }
      </div>
    `;
...

{
  ...
  value: 1,
  message: () => {
    const list = window.__topPopularRecipes;
   
    if (!list || list.length === 0) {
      ensureTopPopularRecipes();

      return `
        <div class="popular-recipe-loading">
          ${initT("popularRecipeLoading")}
        </div>
      `;
    }
  ...
  <div class="popular-recipe-card"
     onclick='openRecipeFromStat(${JSON.stringify(recipe)})'
  >
  ...
  <div class="recipe-info">
    ${initT("popularRecipeInfo")}
  </div>

  <div class="popular-recipe-footer">
    <a href="javascript:void(0)" onclick="showSection('recipes')">
      ${initT("popularRecipeGoTo")}
    </a>
  </div>
`;
  }
...

{
  ...
  value: 1,
  message: () => {
    const next = getNextLessonTitleRandom();
    ...
    return `
      <div class="next-lesson-card">
        ...
        <div class="next-lesson-title">
          ${initT("nextLessonTitleNewsBox")}
          <strong>${next.title}</strong>
        </div>

        <div class="next-lesson-footer">
          <a href="javascript:void(0)" onclick="handleGoToLessons()">
           ${initT("goToLessons")}
          </a>
        </div>
...
if (
  randomStat.key === "pet_tip" ||
  randomStat.key === "celebrity_quote" ||
  randomStat.key === "success_story"
) {
...
if (randomStat.key === "popular_recipe") {
  textEl.innerHTML = comparison;

  return;
}

if (randomStat.key === "next_lesson") {
  textEl.innerHTML = comparison;
  return;
}
...
async function loadTopPopularRecipes() {
  const lang = window.appState?.lang || localStorage.getItem("lang") || "en";
  ...
  const { data, error } = await supabase
    ...
    `)
    .eq("recipes.lang", lang)   // 🔥 important filter
    .limit(1000);
  ...
  instructions: r.description, // or r.instructions if you have it
  likes: 0
};
...
let __popularRecipesPromise = null;

function ensureTopPopularRecipes() {
  if (!__popularRecipesPromise) {

    __popularRecipesPromise = loadTopPopularRecipes().then(data => {

      window.__topPopularRecipes = data;
  ...
  return __popularRecipesPromise;
}

window.openRecipeFromStat = function(recipe) {
  showRecipeModal({
...
function getNextLessonTitleRandom() {
  const profile = currentProfile;
  if (!profile) return null;

  const courses = globalLessonsToRender || {};
  const progress = profile.extra_lesson || {};

  const candidates = [];

  // 🔹 collect ALL next available lessons from all paths
  for (const courseId of Object.keys(courses)) {
    const lessons = courses[courseId];
    const completed = progress[courseId] || [];

    for (let i = 0; i < lessons.length; i++) {
      const lesson = lessons[i];

      const enTitle = lesson?.title?.en;
      if (!enTitle) continue;

      if (!completed.includes(enTitle)) {
        candidates.push({
          courseId,
        ...
        // only push first available per course (prevents bias)
        break;
      }
    }
  }
  ...
  // 🎲 random across all paths
  return candidates[Math.floor(Math.random() * candidates.length)];
}

window.handleGoToLessons = function () {
  showSection('learn');
...
//--------------------------
// EXTERNAL LINK HANDLER (DEBUG MODE)
//--------------------------

const INTERNAL_DOMAINS = [
  'vegan-buddy.vercel.app',
  'localhost'
];

// ✅ Detect native environment safely
const isNativeApp = () => {
  return window?.Capacitor?.isNativePlatform?.() === true;
  return result;
};

// ✅ Open URL (web + native safe)
const openExternalUrl = async (url) => {
  ...
  try {;
    ...
    else if (window.Capacitor?.Plugins?.Browser?.open) {
    ...
    console.error('[LINK DEBUG] Failed to open URL:', err);
  }
};
...
const isInternalLink = (url) => {
  try {
    ...
    const result = INTERNAL_DOMAINS.some(domain =>
...
const setupExternalLinkHandler = () => {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (
      ...
      link.getAttribute('target') === '_system'
    ) {
      return;
    }
...
async function addBadges(userId, amount) { console.log(userId, amount)

  // 1️⃣ Get current count from local profile
  const currentBadges = currentProfile?.badge || 0;
  const newCount = currentBadges + amount;
  ...
  // 3️⃣ Update local state (IMPORTANT)
  currentProfile.badge = newCount;

  // 4️⃣ Atomic update in Supabase
  const { error } = await supabase.rpc("increment_badges", {
    user_id: userId,
    amount: amount
  });

  if (error) {
    console.error(helperT("errorUpdatingBadges"), error);

    // ❗ rollback UI if needed
    currentProfile.badge = currentBadges;

    if (badgeShopEl) badgeShopEl.textContent = currentBadges;
    if (badgeProfileEl) badgeProfileEl.textContent = currentBadges;

    return;
  }

  // 5️⃣ Optional: refresh leaderboard
...
function getTodaysLessonFromProfile(profile) {
  const day = profile.day_counter;

  if (day > 5) {
    return basicDCI(profile);
  } else if (day === 0) {
    return introDCI(profile);
  } else if (day === 1) {
    return secondDCI(profile);
  } else if (day === 2) {
    return thirdDCI(profile);
  } else if (day === 3) {
    return gameintroDCI(profile);
  } else if (day === 4) {
    return fourthDCI(profile);
  } else if (day === 5) {
    return fifthDCI(profile);
  }

  return basicDCI(profile);
}

  function basicDCI(profile){
  const defaultGoalOrder = [
    "Protecting animals & animal welfare",
    "Caring for the environment & fighting climate change",
    "Healthy living & wellness",
...
function introDCI(profile) {
  return {
    todayGoal: "Onboarding",
    todayLessonId: 5000,
    todayLesson: onboardingStaticLessons.intro
  };
}

function secondDCI(profile) {
  return getDietBasedOnboardingLesson(profile, 0);
}

function thirdDCI(profile) {
  return getDietBasedOnboardingLesson(profile, 1);
}


function fourthDCI(profile) {
  return getDietBasedSecondOnboardingLesson(profile, 0);
}

function fifthDCI(profile) {
  return getDietBasedSecondOnboardingLesson(profile, 1);
}

function getDietBasedOnboardingLesson(profile, offset) {
  const diet = (profile.diet_preference || "").toLowerCase();

  const flow = onboardingFlow[diet];
  ...
  // ✅ clamp offset so it never breaks
  const safeOffset = Math.min(offset, flow.length - 1);

  const lessonId = flow[safeOffset];
  const lesson = onboardingLessonPool[lessonId];
...
function getDietBasedSecondOnboardingLesson(profile, offset) {
  const diet = (profile.diet_preference || "").toLowerCase();

  const flow = onboardingFlowSecond[diet];
  ...
  const lessonId = flow[safeOffset];
  const lesson = onboardingSecondLessonPool[lessonId];
...
function gameintroDCI(profile) {
  return {
    todayGoal: "Onboarding",
    todayLessonId: 5013,
    todayLesson: onboardingStaticLessons.gameIntro
  };
}

export const onboardingStaticLessons = {
  intro: {
    id: 5000,
    goal: "Onboarding",
    diet: "all",
    title: {
      en: "Your journey is starting",
      es: "Tu viaje está comenzando",
      hu: "Az élményed most kezdődik"
    },
    lesson: {
      en: `
      <p>Well done — let’s start your journey!</p>

      <p>Did you know? Choosing plant-based meals not only reduces harm to animals, but also significantly lowers your carbon footprint.</p>

      <p>That’s why we’ll log your meals every day — and track your real impact over time.</p>

      <p>Based on scientific averages, in just one month, your choices could save:</p>
      ...
      <p><strong>Now, log your first meal 👇</strong></p>
    ...
    es: `
  <p>Bien hecho — empecemos tu experiencia!</p>

<p>¿Sabías que elegir comidas vegetales no solo reduce el daño a los animales, sino que también disminuye significativamente tu huella de carbono?</p>

<p>Por eso registraremos tus comidas cada día — y seguiremos tu impacto real con el tiempo.</p>

<p>Basado en promedios científicos, en solo un mes, tus decisiones pueden salvar y ahorrar:</p>
...
<p><strong>Ahora, registra tu primera comida 👇</strong></p>
...
,
    ...
    hu: `
<p>Szép munka — kezdjük is el!</p>

<p>Tudtad? A növényi alapú étkezés nemcsak csökkenti az állatoknak okozott kárt, hanem jelentősen csökkenti az ökológiai lábnyomodat is.</p>

<p>Ezért minden nap rögzítjük az étkezéseidet — és idővel követjük a valódi hatásodat.</p>

<p>Tudományos átlagok alapján, már egy hónap alatt a döntéseid megmenthetnek és megspórolhatnak:</p>

<ul>
  <li>🐾 ~21 állatot</li>
  <li>🌳 ~15 m² erdőt</li>
  <li>💧 ~19 800 liter vizet</li>
  <li>🌍 ~120 kg CO₂-t</li>
</ul>

<p><strong>Most rögzítsd az első étkezésed 👇</strong></p>
...
<p>You’ve reached your fourth day — great progress 🌱</p>

<p>You’ve already got a taste of the complexity of our food system… and your journey continues.</p>

<p>Today, you unlock a different kind of tracking — the <strong>Game section</strong> 🎮</p>

<p>Here, your progress connects to honor points, achievements, challenges, and titles. You can also choose messages that represent you best — and show them to others.</p>
...
<p>Has llegado a tu cuarto día — gran progreso 🌱</p>

<p>Ya has probado un poco la complejidad de nuestro sistema alimentario… y tu camino continúa.</p>

<p>Hoy desbloqueas un tipo diferente de seguimiento — la <strong>sección de juego</strong> 🎮</p>

<p>Aquí tu progreso se conecta con puntos de honor, logros, desafíos y títulos. También puedes elegir mensajes que te representen — y mostrarlos a los demás.</p>
...
<p>Elérted a negyedik napot — szép haladás 🌱</p>

<p>Már kaptál egy kis ízelítőt az élelmiszer-rendszerünk összetettségéből… és az utad folytatódik.</p>

<p>Ma feloldod a <strong>Játék szekciót</strong> 🎮</p>

<p>Itt a fejlődésed összekapcsolódik becsületpontokkal, teljesítményekkel, kihívásokkal és címekkel. Emellett olyan üzeneteket is választhatsz, amelyek téged képviselnek — és megmutathatod őket másoknak.</p>

<p><strong>Induljunk 🚀</strong></p>
    `
    }
  }
};
...
const submitBtn = document.getElementById("submitBtnDCI");
const supportBtn = document.getElementById("submitAndSupportBtnDCI");
...
if (currentProfile.day_counter === 0) {
  ...
  // 👉 IMPORTANT: trigger animation HERE
  startDailyCheckinAnimation();
...
function renderDailyPet() {
  const petAvatar = document.getElementById("petAvatar");
 // const petMessage = document.getElementById("petMessage");

  if (!petAvatar ) return;

  // 🐾 1. Avatar (from Supabase)
  if (currentProfile.pet_photo) {
    petAvatar.innerHTML = `
      <img src="${currentProfile.pet_photo}" 
           alt="pet" 
           class="pet-image">
    `;
  } else {
    petAvatar.innerHTML = "🐾"; // fallback
  }
}
...
window.starterSubmit = async function starterSubmit() {
  ...
  try {
    const proceed = await handleStreakLogicInline();
    if (!proceed) return;
    ...
    enableDailyCheckinButtons();
  }
};

document.getElementById("submitBtnDCI").onclick = async () => {
  disableDailyCheckinButtons();
  ...
  try {
    ...
    enableDailyCheckinButtons(); // 🔥 ALWAYS runs
  }
};

document.getElementById("submitAndSupportBtnDCI").onclick = async () => {
  disableDailyCheckinButtons();
...
// 2. DOM (normal flow)
  ...
  mealValue = getValidatedMeal();
if (mealValue === null) return false;
}
...
updatedProfile.day_counter += 1;
const reachedGameSection = updatedProfile.day_counter === 4;
updatedProfile.streak = (updatedProfile.streak || 0) + 1;

if (mealValue === 4) {
  updatedProfile.vegan_days = (updatedProfile.vegan_days || 0) + 1;
  updatedProfile.badge = (updatedProfile.badge || 0) + 5;
}

updatedProfile.animals_saved = (updatedProfile.animals_saved || 0) + impactIncrement.animals_saved;
updatedProfile.forest_saved = (updatedProfile.forest_saved || 0) + impactIncrement.forest_saved;
updatedProfile.water_saved = (updatedProfile.water_saved || 0) + impactIncrement.water_saved;
updatedProfile.co2_saved = (updatedProfile.co2_saved || 0) + impactIncrement.co2_saved;

updatedProfile.last_checkin_date = new Date().toISOString().split("T")[0];

// lessons
if (!updatedProfile.lesson_progress) updatedProfile.lesson_progress = [];
if (!updatedProfile.completed_lessons) updatedProfile.completed_lessons = [];

if (!updatedProfile.completed_lessons.includes(todayLessonId)) {
  updatedProfile.completed_lessons.push(todayLessonId);
  updatedProfile.lesson_progress.push(todayLessonId);
}

updatedProfile.last_lesson = { goal: todayGoal, lessonId: todayLessonId };
...
function getValidatedMeal() {
  ...
  return parseInt(mealAnswer.value);
}

async function handleStreakLogicInline() {

  // ✅ GET ONCE (no double calls anymore)
  const mealValue = getValidatedMeal();
  if (mealValue === null) return false;
  ...
  // ✅ CASE 1: no penalty
  if (badgeCost === 0 || profile.streak === 0) return true;
  ...
  if (userBadges >= badgeCost) {
    const confirmKeep = confirm(
      helperT("confirmSaveStreak", { badgeCost })
    );
  ...
  alert(helperT("notEnoughBadges", { badgeCost, missing }));
  ...
  // ✅ SAVE pending (mealValue already validated)
  await supabase
    .from("profiles")
    .update({
...
function startDailyCheckinAnimation() {

  const lessonEl = document.getElementById("dailyLessonDCI");
...
const lessonText = stripHTML(raw);

  lessonEl.innerHTML = "";

  typeText(lessonEl, lessonText, 20, () => {
...
window.showRecipeModal = function showRecipeModal(recipeData) {
...
async function initNotificationSettings(userId) {
  await loadNotificationSettings(userId);
  bindNotificationToggles(userId);
}
...
async function loadNotificationSettings(userId) {
  ...
  document.getElementById("toggleNewMessage").checked =
    prefs.pushNewMessage ?? true;

  document.getElementById("toggleFriendRequest").checked =
    prefs.pushFriendRequest ?? true;

  document.getElementById("toggleCommunityEvent").checked =
    prefs.pushCommunityEvent ?? true;

  document.getElementById("toggleDailyCheckin").checked =
    prefs.dailyCheckinReminder ?? true;

  document.getElementById("toggleWeeklyVote").checked =
    prefs.weeklyVoteReminder ?? true;

  document.getElementById("toggleEncouragement").checked =
    prefs.sendEncouragement ?? true;
}
...
async function updatePreference(userId, key, value) {
  ...
  const current = data?.notification_preferences || {};
...
function bindNotificationToggles(userId) {
  document.getElementById("toggleNewMessage")
    .addEventListener("change", (e) =>
      updatePreference(userId, "pushNewMessage", e.target.checked)
    );

  document.getElementById("toggleFriendRequest")
    .addEventListener("change", (e) =>
      updatePreference(userId, "pushFriendRequest", e.target.checked)
    );

  document.getElementById("toggleCommunityEvent")
    .addEventListener("change", (e) =>
      updatePreference(userId, "pushCommunityEvent", e.target.checked)
    );

  document.getElementById("toggleDailyCheckin")
    .addEventListener("change", (e) =>
      updatePreference(userId, "dailyCheckinReminder", e.target.checked)
    );

  document.getElementById("toggleWeeklyVote")
    .addEventListener("change", (e) =>
      updatePreference(userId, "weeklyVoteReminder", e.target.checked)
    );

  document.getElementById("toggleEncouragement")
    .addEventListener("change", (e) =>
      updatePreference(userId, "sendEncouragement", e.target.checked)
    );
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    await initNotificationSettings(currentProfile.id);
...
const characterTips = {
  ...
  pet: {
    en: [
    `Did you know? 🧠 \n\nChimpanzees outperform us in short-term memory tests! In some tasks, they recall number sequences faster and more accurately than humans. \n\n<a href="https://www.youtube.com/watch?v=cK-dtUTb8ME" target="_blank">Check this video out!</a>.`,
    ...
    es: [
      `¿Sabías que? 🧠 \n\nLos chimpancés superan a los humanos en pruebas de memoria a corto plazo. En algunos tests recuerdan secuencias numéricas con mayor precisión y rapidez. \n\n<a href="https://www.youtube.com/watch?v=cK-dtUTb8ME" target="_blank">Mira este vídeo!</a>.`,
    ...
    hu: [
      `Tudtad? 🧠 \n\nA csimpánzok jobban teljesítenek a rövid távú memória tesztekben, mint az emberek. Egyes feladatokban gyorsabban és pontosabban jegyzik meg a számsorokat. \n\n<a href="https://www.youtube.com/watch?v=cK-dtUTb8ME" target="_blank">Nézd meg ezt a videót!</a>.`,
